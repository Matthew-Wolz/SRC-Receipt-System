...
function AthleticTapeForm({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/submit-athletic-tape", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(data || "Failed to submit Athletic Tape.");
      }

      setSuccess("Athletic Tape submitted successfully!");
      setFormData({
        guestName: "",
        staffInitials: "",
        emailReceipt: false,
...
export default AthleticTapeForm;