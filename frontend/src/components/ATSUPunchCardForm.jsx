...
function ATSUPunchCardForm({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/submit-atsu-punch-card", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(data || "Failed to submit ATSU Punch Card.");
      }

      setSuccess("ATSU Punch Card submitted successfully!");
      setFormData({
        guestName: "",
        staffInitials: "",
        emailReceipt: false,
...
export default ATSUPunchCardForm;